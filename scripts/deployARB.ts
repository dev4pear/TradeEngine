...
async function main() {
  ...
  const PairsAndFeedsJson = JSON.parse(
    fs.readFileSync("./PairsAndFeedsARB.json").toString()
  );
  const RouterAddressJson = JSON.parse(
    fs.readFileSync("./PCSRouterAddressARB.json").toString()
  );
  let symbols = [];
  let feeds = [];
  let token1 = [];
  ...
  console.log(RouterAddressJson.address, symbols, feeds, token1, token2);
  // let finalString = "";
  // const TradingBot = await ethers.getContractFactory("TradingBotARB");
  // const bot = await TradingBot.deploy(RouterAddressJson.address, symbols, feeds, token1, token2);
