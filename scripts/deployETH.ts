...
async function main() {
  ...
  const PairsAndFeedsJson = JSON.parse(
    fs.readFileSync("./PairsAndFeedsETH.json").toString()
  );
  const RouterAddressJson = JSON.parse(
    fs.readFileSync("./PCSRouterAddressETH.json").toString()
  );
  let symbols = [];
  let feeds = [];
  let token1 = [];
  ...
  // let finalString = "";
  // // console.log(RouterAddressJson.address, symbols, feeds, token1, token2);
  // const TradingBot = await ethers.getContractFactory("TradingBotETH");
