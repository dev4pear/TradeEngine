...
describe("Testing Suite for Limit Orders / Stop Loss / Take Profits Smart Contract System", function () {
    ...
    describe("Single Transactions", function () {
         ...
         it("Should buy some USDC from PancakeSwap using Native Token (ETH) and Uniswap Router", async function () {
      ...
      describe("Transactions with different pairs", function () {
        ...
        it("Should retrieve price feed data from Chainlink for MATIC/USDC", async function () {
          const response = await TradingBotContract.getChainlinkDataFeedLatestAnswer.staticCall(MATICUSDCFeed);
          expect(response).to.be.a('bigint');
        });

        it("Should place a trade swapping to USDC with 100 MATIC, with Stop Loss and Take Profits Exiting back to USDC", async function () {
        ...
        it("Should place a trade swapping to USDC with 100 MATIC, with qualifying TakeProfit to USDC and execute that trade.", async function () {
          const initialBalance = await MATICContract.balanceOf(owner);
          const resp = await IERC20MATICContract.approve(TradingBotContract.getAddress(), 100n*PRECISIONMATIC);
        ...
        it("Should place a limit order with 100 MATIC to USDC", async function () {
          const initialBalance = await MATICContract.balanceOf(owner);
          const resp = await IERC20MATICContract.approve(TradingBotContract.getAddress(), 100n*PRECISIONMATIC);
        ...
        it("Should place a limit order with 100 MATIC with qualifying trigger and execute that trade.", async function () {
          const initialBalance = await MATICContract.balanceOf(owner);
