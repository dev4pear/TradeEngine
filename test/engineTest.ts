...
const TradingBotJson = fs.readFileSync("./artifacts/contracts/LimitOrderEngine.sol/TradingBot.json");
const IERC20Json = fs.readFileSync("./artifacts/@openzeppelin/contracts/token/ERC20/IERC20.sol/IERC20.json");
const PairsAndFeedsJson = JSON.parse(fs.readFileSync("./PairsAndFeeds.json").toString());
...
//const ETHAddress = PairsAndFeedsJson[14].asset2;
//const BTCETHFeed = PairsAndFeedsJson[14].feed;
...
let IERC20ETHContract: Contract;
...
describe("Testing Suite for Limit Orders / Stop Loss / Take Profits Smart Contract System", function () {
    ...
    before(async function() {
      ...
      ETHContract = new ethers.Contract(ETHAddress, minABI, owner);
      IERC20ETHContract = new ethers.Contract(ETHAddress, JSON.parse(IERC20Json.toString()).abi, owner);
    });

    describe("Single Transactions", function () {
      
        it("Should buy some USDC from PancakeSwap using Native Token (BNB) and Pancake Router", async function () {
          const initialBalance = await USDCContract.balanceOf(owner);
          const tx = await TradingBotContract.swapNativeToToken.send(USDCAddress,10000,{value: ethers.parseEther("0.5")});
          tx.wait();
        ...
        it("Should place a trade buying BTCB with 500 USDC, with Stop Loss and Take Profits Exiting back to USDC", async function () {
          ...
          const tx = await TradingBotContract.enterNewTrade.send(BTCUSDCFeed, USDCAddress, BTCAddress, (500n)*PRECISION, 1, 6786775261845, 6186775261845);
          tx.wait();
        ...
        it("Should place a trade buying BTCB with 500 USDC with qualifying TakeProfit to USDC and execute that trade.", async function () {
          const initialBalance = await USDCContract.balanceOf(owner);
          const resp = await IERC20Contract.approve(TradingBotContract.getAddress(), (500n)*PRECISION);
          const tx = await TradingBotContract.enterNewTrade.send(BTCUSDCFeed, USDCAddress, BTCAddress, (500n)*PRECISION, 10000, 6386775261845, 6186775261845);
          tx.wait();
          const response2 = await TradingBotContract.getCurrentTrades.staticCall();
          const initialLength = response2.length;
          expect(response2[response2.length - 1]).to.be.a('bigint');
        ...
        it("Should place a limit order with 500 USDC to buy BTCB", async function () {
          const initialBalance = await USDCContract.balanceOf(owner);
          const resp = await IERC20Contract.approve(TradingBotContract.getAddress(), (500n)*PRECISION);
          const tx = await TradingBotContract.enterNewLimit.send(BTCUSDCFeed, USDCAddress, BTCAddress, (500n)*PRECISION, 10000, 6186775261845);
          tx.wait();
          const response2 = await TradingBotContract.getCurrentTrades.staticCall();
          expect(response2[response2.length - 1]).to.be.a('bigint');
        });
        ...
        it("Should place a limit order with 500 USDC with qualifying trigger and execute that trade.", async function () {
          ...
          const resp = await IERC20Contract.approve(TradingBotContract.getAddress(), (500n)*PRECISION);
          const tx = await TradingBotContract.enterNewLimit.send(BTCUSDCFeed, USDCAddress, BTCAddress, (500n)*PRECISION, 10000, 6586775261845);
          tx.wait();
          const response2 = await TradingBotContract.getCurrentTrades.staticCall();
          const initialLength = response2.length;
          expect(response2[response2.length - 1]).to.be.a('bigint');
      ...
      describe("Multiple Wallets and Transactions", function () {
        ...
        it("Should place 500 USDC trade with Account 1", async function () {
          const initialBalance = await USDCContract.balanceOf(owner);
          const resp = await IERC20Contract.approve(TradingBotContract.getAddress(), 500n*PRECISION);
          const tx = await TradingBotContract.enterNewTrade.send(BTCUSDCFeed, USDCAddress, BTCAddress, 500n*PRECISION, 10000, 6786775261845, 6186775261845);
          tx.wait();
          const response2 = await TradingBotContract.getCurrentTrades.staticCall();
          expect(response2[response2.length - 1]).to.be.a('bigint');
        });
        ...
        it("Should place 500 USDC trade with Account 2", async function () {
          ...
          const tx2 = await TradingBotContract2.enterNewTrade.send(BTCUSDCFeed, USDCAddress, BTCAddress, 500n*PRECISION, 10000, 6786775261845, 6186775261845);
          tx2.wait();
        ...
        it("Should place 500 USDC trade with Account 3", async function () {
          ...
          const tx3 = await TradingBotContract3.enterNewTrade.send(BTCUSDCFeed, USDCAddress, BTCAddress, 500n*PRECISION, 10000, 6786775261845, 6186775261845);
          tx3.wait();
      ...
      describe("Transactions with different pairs", function () {
        ...
        it("Should retrieve price feed data from Chainlink for BTC/ETH", async function () {
          const response = await TradingBotContract.getChainlinkDataFeedLatestAnswer.staticCall(BTCETHFeed);
          expect(response).to.be.a('bigint');
        });

        it("Should place a trade buying BTCB with 1 ETH, with Stop Loss and Take Profits Exiting back to ETH", async function () {
          const initialBalance = await ETHContract.balanceOf(owner);
          const resp = await IERC20ETHContract.approve(TradingBotContract.getAddress(), 1n*PRECISION);
          const tx = await TradingBotContract.enterNewTrade.send(BTCETHFeed, ETHAddress, BTCAddress, 1n*PRECISION,10000, 6786775261845, 6186775261845);
          tx.wait();
          const response2 = await TradingBotContract.getCurrentTrades.staticCall();
          expect(response2[0]).to.be.a('bigint');
        });
        ...
        it("Should place a trade buying BTCB with 1 ETH with qualifying TakeProfit to USDC and execute that trade.", async function () {
          const initialBalance = await ETHContract.balanceOf(owner);
          const resp = await IERC20ETHContract.approve(TradingBotContract.getAddress(), 1n*PRECISION);
          const tx = await TradingBotContract.enterNewTrade.send(BTCETHFeed, ETHAddress, BTCAddress, 1n*PRECISION, 10000, 6386775261845, 6186775261845);
          tx.wait();
          const response2 = await TradingBotContract.getCurrentTrades.staticCall();
          const initialLength = response2.length;
          expect(response2[response2.length - 1]).to.be.a('bigint');
          const newBalance = await ETHContract.balanceOf(owner);
          expect(newBalance).to.be.lessThan(initialBalance);
          
          const tx2 = await TradingBotContract.executeTrade.send(response2[response2.length - 1]);
          tx2.wait();
    
          const finalBalance = await ETHContract.balanceOf(owner);
          expect(finalBalance).to.be.greaterThan(newBalance);
          const response3 = await TradingBotContract.getCurrentTrades.staticCall();
          const newLength = response3.length;
          expect(newLength).to.equal(initialLength - 1);
        ...
        it("Should place a limit order with 1 ETH to buy BTCB", async function () {
          const initialBalance = await ETHContract.balanceOf(owner);
          const resp = await IERC20ETHContract.approve(TradingBotContract.getAddress(), 1n*PRECISION);
          const tx = await TradingBotContract.enterNewLimit.send(BTCETHFeed, ETHAddress, BTCAddress, 1n*PRECISION, 10000, 6186775261845);
          tx.wait();
          const response2 = await TradingBotContract.getCurrentTrades.staticCall();
          expect(response2[response2.length - 1]).to.be.a('bigint');
        });
        ...
        it("Should place a limit order with 500 USDC with qualifying trigger and execute that trade.", async function () {
          ...
          const initialBTCBalance = await BTCBContract.balanceOf(owner);
          const resp = await IERC20ETHContract.approve(TradingBotContract.getAddress(), 1n*PRECISION);
          const tx = await TradingBotContract.enterNewLimit.send(BTCETHFeed, ETHAddress, BTCAddress, 1n*PRECISION, 10000, 21911481786026600000n);
          tx.wait();
          const response2 = await TradingBotContract.getCurrentTrades.staticCall();
          const initialLength = response2.length;
          expect(response2[response2.length - 1]).to.be.a('bigint');
         ...
         it("Should refuse to place a trade when a feed address from unapproved pair is attempted", async function () {
          const initialBalance = await ETHContract.balanceOf(owner);
          const resp = await IERC20ETHContract.approve(TradingBotContract.getAddress(), 1n*PRECISION);
          expect(TradingBotContract.enterNewTrade.send("0xb7Ed5bE7977d61E83534230f3256C021e0fae0B6", ETHAddress, BTCAddress, 1n*PRECISION, 10000, 6786775261845, 6186775261845)).to.be.revertedWithCustomError(TradingBotContract, "TradingPairCurrentlyNotOffered");
        });

        it("Should refuse to place a trade when the wrong tokens are provided alongisde the feed address", async function () {
          await new Promise(resolve => setTimeout(resolve, 3000));
          expect(TradingBotContract.enterNewTrade.send(BTCETHFeed, USDCAddress, BTCAddress, 1n*PRECISION, 10000, 6786775261845, 6186775261845)).to.be.revertedWithCustomError(TradingBotContract, "ContractCallProvidedWrongTokenPairOrPriceFeed");
        }); 
      }); 
});